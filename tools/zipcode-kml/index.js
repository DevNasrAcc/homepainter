...
const inputSourceFile = __dirname + '/allZip.kml';
const outputFilePrefix = 'zip';
...
<kml xmlns:gx="http://www.google.com/kml/ext/2.2" xmlns:atom="http://www.w3.org/2005/Atom"
  ...
  <Document>
    ...
    <name>cb_2018_us_zcta510_500k</name>
    ...
    <Schema name="cb_2018_us_zcta510_500k" id="kml_schema_ft_cb_2018_us_zcta510_500k">
    ...
    <Folder id="kml_ft_cb_2018_us_zcta510_500k">
      <name>cb_2018_us_zcta510_500k</name>
...
parser.parseString(sharedKml, function (err1, result1) {
  ...
  fs.readFile(inputSourceFile, function (err, data) {
    parser.parseString(data, function (err, result) {
      const doc = result.kml.Document[0].Folder[0].Placemark;
      for (let i = 0; i < doc.length; i++) {
        const zip = doc[i].ExtendedData[0].SchemaData[0].SimpleData[0]._;
        top.kml.Document[0].Folder[0].Placemark = doc[i];
        const xml = builder.buildObject(top);

        fs.writeFileSync(__dirname + '\\all-zips-hp\\' + outputFilePrefix + zip + '.kml', xml);
      }
